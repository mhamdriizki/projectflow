...
export function TaskBoard({ tasks }: { tasks: Task[] }) {
  ...
  function handleStatusChange(taskId: string, status: TaskStatus) {
    // startTransition wajib: setOptimisticStatus itu non-urgent update,
    // React boleh interrupt kalau ada update lain yang lebih urgent
    // masuk duluan. Tanpa transition ini, React ngelempar error
    // "An optimistic state update occurred outside a transition".
    startTransition(async () => {
      setOptimisticStatus({ taskId, status });
      await updateTaskStatus(taskId, status);
      // Kalau updateTaskStatus gagal/throw, React otomatis rollback
      // optimisticTasks balik ke `tasks` (props asli dari server) —
      // gak perlu manual try/catch buat balikin UI.
    });
  }
